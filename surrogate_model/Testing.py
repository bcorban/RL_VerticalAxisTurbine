...
for i,index in enumerate(test_index[:-1]):
    ...
    R2_test= sklearn.metrics.r2_score(Y_truth[::tau,1][1:],Y_pred[:,1])
    ...
    if plot_bool:
        if i<number_of_figs:
            ...
            ax[0,0].plot([n/T for n in range(len(Y_truth))],Y_truth[:,0])
            ax[0,0].plot([n*tau/T for n in range(nsteps-1)],Y_pred[:,0])
            ax[0,0].plot([n/T for n in range(len(Y_pred2))],Y_pred2[:,0])
            
            ax[0,1].set_title('Cp')
            ax[0,1].plot([n/T for n in range(len(Y_truth))],Y_truth[:,1])
            ax[0,1].plot([n*tau/T for n in range(nsteps-1)],Y_pred[:,1])
            ax[0,1].plot([n/T for n in range(len(Y_pred2))],Y_pred2[:,1])
            
            ax[1,0].set_title('Cr')
            ax[1,0].plot([n/T for n in range(len(Y_truth))],Y_truth[:,2])
            ax[1,0].plot([n*tau/T for n in range(nsteps-1)],Y_pred[:,2])
            ax[1,0].plot([n/T for n in range(len(Y_pred2))],Y_pred2[:,2])
            
            ax[1,1].set_title('Cm')
            ax[1,1].plot([n/T for n in range(len(Y_truth))],Y_truth[:,3])
            ax[1,1].plot([n*tau/T for n in range(nsteps-1)],Y_pred[:,3])
            ax[1,1].plot([n/T for n in range(len(Y_pred2))],Y_pred2[:,3])
...
for i in best:
    ...
    R2_test= sklearn.metrics.r2_score(Y_truth[:-2*tau:tau,1],Y_pred[:,1])
    print(f"test n°{i} : R2 = {R2_test}")
    R2_memory.append(R2_test)
    Cp_mean_memory.append(Y_truth[:,1].mean())
